...
jQuery.extend({
	...
	touchEvent : function(origin, range, touch, press){
		...
		var onPress = {
			...
			killTimer : function(){
				this.killExeculteId = false;
				this.execulteStop   = false;
				this.isExecutePress = false;
				clearInterval(this.execulteId);
			},
			...
			executePress : function(){
				...
				_this.isExecutePress = true;
			},
			
			executeComplate : function(){
				this.killExeculteId = false;
				this.execulteStop   = false;
				this.isExecutePress = false;
			}
		...
		$(origin, guess).unbind('touchmove').bind('touchmove', function(e){
			...
			if(changeScreenX > 5 || changeScreenY > 5){
				onPress.execulteStop = true;
			}
		...
		$(origin, guess).unbind('touchend').bind('touchend', function(e){
			/**
			 * 没有执行过长按事件则执行 touchend 事件
			 */
			if(!onPress.isExecutePress){
				var changeScreenX = Math.abs(startScreenX-moveScreenX);
				var changeScreenY = Math.abs(startScreenY-moveScreenY);
